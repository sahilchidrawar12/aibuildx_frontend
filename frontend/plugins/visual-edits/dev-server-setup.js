...
function setupDevServer(config) {
  config.setupMiddlewares = (middlewares, devServer) => {
    ...
    const isAllowedOrigin = (origin) => {
      ...
      // Example: if (origin.match(/^https:\/\/([a-zA-Z0-9-]+\.)*yourdomain\.com$/)) {
      //   return true;
      // }
      ...
      try {
        ...
        Object.entries(changesByFile).forEach(([fileName, fileChanges]) => {
          ...
          try {
            ...
            execSync(`git -c user.name="visual-edit" -c user.email="noreply@localhost" add "${targetFile}"`);
            execSync(`git -c user.name="visual-edit" -c user.email="noreply@localhost" commit -m "visual_edit_${timestamp}"`);
          } catch (gitError) {
